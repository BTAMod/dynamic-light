...
public class DynamicLight implements ModInitializer {
    ...
    private void tickEntity(net.minecraft.client.Minecraft mc)
    {
        for(int j = 0; j < DynamicLightDispatcher.itemArray.size(); j++) // loop the PlayerTorch List
        {
            DynamicLightSourceEntity torchLoopClass = (DynamicLightSourceEntity) DynamicLightDispatcher.itemArray.get(j);
    ...
    private void TickPlayerEntity(net.minecraft.client.Minecraft mc, DynamicLightSourceEntity torchLoopClass, EntityPlayer entPlayer)
    {
    ...
    private int GetPlayerArmorLightValue(DynamicLightSourceEntity torchLoopClass, EntityPlayer entPlayer, int oldbrightness)
    {
    ...
    private void TickItemEntity(net.minecraft.client.Minecraft mc, DynamicLightSourceEntity torchLoopClass, Entity torchent)
    {
        torchLoopClass.setTorchPos(mc.theWorld, (float)torchent.x, (float)torchent.y, (float)torchent.z);
    ...
    private void UpdateBurningEntities(net.minecraft.client.Minecraft mc)
    {
        for(int k = 0; k < mc.theWorld.loadedEntityList.size(); k++) // we loop ALL entities
        {
            ...
            if (shouldEmitLight(tempent))
            {
                ...
                DynamicLightSourceEntity torchent = DynamicLightDispatcher.GetTorchForEntity(tempent);
                ...
                if (torchent == null) // if it is on fire and not yet a playertorch...
                {
                    ...
                    DynamicLightSourceEntity newtorch = new DynamicLightSourceEntity(tempent); // add one with torch data!
    ...
    private void updateEntity(World worldObj)
    {
        ...
        for(int l = 0; l < tempList.size(); l++) // now to loop the remainder of the fresh list, the NEW lights
        {
            ...
            DynamicLightSourceEntity newtorch = new DynamicLightSourceEntity(newent);
            DynamicLightDispatcher.AddEntity(newtorch);
